...
lonList = []
latList = []
for_total_url = 'http://apis.data.go.kr/B553077/api/open/sdsc2/storeListInDong?divId=ctprvnCd&key={}&indsSclsCd=D03A01&divId=ctprvnCd&numOfRows=1000&pageNo=1&type=xml&serviceKey=%2BJTG2GnVWVXZAxaul97F7f9DHnabKZ5Oiaw5eMiZJ1jGKGxyPSNm89FrSrS9pq5%2FLD5DMiDRMT2JJFp6AnK9eQ%3D%3D'.format(city_code)
...
for i in range(1, (int(total)//1000 + 2)): # total을 몫값 +1, for문 때문에 +1, 총 +2
...
hash_one = time.time() - start_time
print(time.strftime("%H:%M:%S", time.gmtime(hash_one)))

# -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# 5 - 단지코드, 단지명을 kaptCode_list 와 kaptName_list에 저장 (뒤에 kaptName_list 가 다시한번 선언되니 문제되면 삭제)
# -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

kaptCode_list = [] # 서울시 모든 단지코드 리스트
# kaptName_list = [] # 서울시 모든 단지명 리스트

for_total2_url = 'http://apis.data.go.kr/1613000/AptListService2/getSidoAptList?sidoCode={}&numOfRows=1000&pageNo=1&ServiceKey=%2BJTG2GnVWVXZAxaul97F7f9DHnabKZ5Oiaw5eMiZJ1jGKGxyPSNm89FrSrS9pq5%2FLD5DMiDRMT2JJFp6AnK9eQ%3D%3D'.format(city_code)
...
for i in range(1, (int(total2)//1000 + 2)):
    url = 'http://apis.data.go.kr/1613000/AptListService2/getSidoAptList?sidoCode={}&numOfRows=1000&pageNo={}&ServiceKey=%2BJTG2GnVWVXZAxaul97F7f9DHnabKZ5Oiaw5eMiZJ1jGKGxyPSNm89FrSrS9pq5%2FLD5DMiDRMT2JJFp6AnK9eQ%3D%3D'.format(city_code, i)
...
hash_two = time.time() - start_time
print(time.strftime("%H:%M:%S", time.gmtime(hash_two)))
...
hash_three = time.time() - start_time
print(time.strftime("%H:%M:%S", time.gmtime(hash_three)))
